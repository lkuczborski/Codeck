    ...
    private static func highlightMarkdown(_ code: String) -> String {
        ...
        return code.split(separator: "\n", omittingEmptySubsequences: false).enumerated().map { index, line in
            ...
            if markdownFenceMarker(in: rawLine) != nil {
                ...
                rendered = span("syntax-punctuation", String(rawLine.prefix(1))) + spanHTML(
                    "syntax-comment",
                    highlightMarkdownInline(String(rawLine.dropFirst(1)))
                )
            } else if let markerLength = markdownListMarkerLength(in: rawLine) {
                rendered = span("syntax-punctuation", String(rawLine.prefix(markerLength))) +
                    highlightMarkdownListContent(String(rawLine.dropFirst(markerLength)))
