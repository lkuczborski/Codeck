...
private final class CodeckMCPServer {
    ...
    while let line = readLine(strippingNewline: true) {
      ...
      do {
        ...
        if let batch = message as? [[String: Any]] {
          ...
          if let response = handleMessage(object) {
            writeJSON(response)
          }
        } else {
  ...
  private func resourceContent(for uri: String) throws -> (mimeType: String, text: String) {
    ...
    case "outline":
      return ("application/json", try jsonText(DeckResponse(path: url.path, deck: DeckDescription(deck), markdown: nil)))
    case "slide":
      ...
      return ("application/json", try jsonText(SlideResponse(path: url.path, index: index, slide: deck.slides[index])))
    default:
...
private struct PathAccessGuard {
...
private struct DeckResponse: Encodable {
  let path: String
...
private struct ValidationResponse: Encodable {
  let path: String
  let valid: Bool
  let warnings: [String]
  let deck: DeckDescription
...
private struct DeckDescription: Encodable {
  let theme: String
...
private struct CodexSettingsDescription: Encodable {
  let model: String
...
private struct SlideResponse: Encodable {
  let path: String
  let index: Int
...
private struct SlideDescription: Encodable {
  let index: Int
  let title: String
  let summary: String
...
private struct CodexBlockDescription: Encodable {
  let id: String
  let title: String
  let model: String?
  let reasoning: String?
...
private func optionalBool(_ arguments: [String: Any], _ key: String) -> Bool? {
...
private func booleanSchema(_ description: String) -> [String: Any] {
  ["type": "boolean", "description": description]
}

private func arraySchema(items: [String: Any]) -> [String: Any] {
